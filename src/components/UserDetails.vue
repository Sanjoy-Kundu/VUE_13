<script setup>
    import {ref,reactive, computed, watch} from "vue"
    const users = reactive({
        name: "",
        profileImage:"",
        birthday:'',
        age:0,
        email:"",
        description:""
    })

    const calculateAge = computed(()=>{
        if(users.birthday){
            const userInputAge = new Date(users.birthday);
            const userInputYear = (userInputAge.getFullYear());
            const currentYearDate = new Date();
            const currentYear = currentYearDate.getFullYear();
            const result = currentYear - userInputYear;
              return result
             
        }
    })

    //check votar 
    const checkVotar = computed(()=>{
        if(users.birthday){
            const userInputAge = new Date(users.birthday);
            const userInputYear = (userInputAge.getFullYear());
            const currentYearDate = new Date();
            const currentYear = currentYearDate.getFullYear();
            const YourAge  = currentYear - userInputYear;
            if(YourAge<18){
                    return "User eligible not for voting";
                }else{
                    return "user eligible for voting";
                }
        }
    })

    watch(()=>users.age, (newAge, oldAge) =>{
        if(newAge){
            users.birthday = new Date().getFullYear() - newAge;
        }
    })
      
       
</script>
<template>
    <h2 class="text-center">This is user details component</h2>
    <div class="user-details-show">
        <h2 class="text-center">Your Date of Bith: {{ users.birthday }}</h2>
        <!-- <h2 class="text-center" v-if="users.birthday">User Age is {{ calculateAge }}</h2> -->
        <h3 class="text-center">{{ checkVotar }}</h3>
        <!-- <h3 class="text-center text-center">Computed task Task Three</h3>
        
             -->
    </div>

    <div class="user-form w-50 mx-auto">
        <div class="mb-3">
        <label for="" class="form-label">Date of Birth</label>
        <input type="date" class="form-control" id="" v-model="users.birthday">
        </div>
    </div>
    <div class="user-form w-50 mx-auto">
        <div class="mb-3">
        <label for="" class="form-label">Your Age</label>
        <input type="number" readonly disabled class="form-control" id="" v-model="calculateAge">
        </div>
    </div>
    <div class="card mx-auto" style="width: 24rem;">
                <img src="data:image/jpeg;base64," class="card-img-top" alt="...">
                <div class="card-body">
                    <p v-if="users.age < 18">Your are not  eligible</p>
                    <p v-if="users.age >= 18">Your are  eligible</p>
                </div>
                </div>
    <div class="user-form w-50 mx-auto">
        <div class="mb-3">
        <label for="" class="form-label">Computed</label>
        <input type="number" class="form-control" id="" v-model="users.age">
        </div>
    </div>
  


</template>
<style scoped></style>